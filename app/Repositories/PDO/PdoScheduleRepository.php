...
class PdoScheduleRepository implements ScheduleRepositoryInterface {
    ...
    public function save(ScheduleEvent $event): bool {
        $stmt = $this->db->prepare(
            "INSERT INTO schedule (user_id, event_title, event_date, notes, color, location) 
    ...
    public function update(ScheduleEvent $event): bool {
        $stmt = $this->db->prepare(
            "UPDATE schedule SET event_title = :title, event_date = :date, notes = :notes, color = :color, location = :location 
